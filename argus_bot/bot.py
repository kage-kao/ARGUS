...
class Pending:
    ...
    buzzcast_id: str | None = None   # BuzzCast streamer id (if recording one)
    nick: str | None = None          # BuzzCast streamer nickname
...
def _subject_line(buzzcast_id: str | None, nick: str | None) -> str | None:
    """Строка с инфо о записываемом стримере (ник + ID), если это BuzzCast."""
    if not buzzcast_id:
        return None
    return f"👤 Стример: <b>{nick or '—'}</b> (ID: <code>{buzzcast_id}</code>)"
...
def _kb_hoster() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="🟣 Gofile (без лимита)", callback_data="h:gofile")],
        [
            InlineKeyboardButton(text="🚀 Ranoz + Tempshare", callback_data="h:both"),
            InlineKeyboardButton(text="🌐 Все три", callback_data="h:all"),
        ],
...
def _kb_quality() -> InlineKeyboardMarkup:
    ...
    # Режим «сжать до N МБ» — по 2 кнопки в ряд
    size_btns = [
        InlineKeyboardButton(
            text=("🎯 1 ГБ" if mb == 1000 else f"🎯 {mb} МБ"),
            callback_data=f"q:size:{mb}",
        )
        for mb in config.SIZE_TARGETS_MB
    ]
    for i in range(0, len(size_btns), 2):
        rows.append(size_btns[i:i + 2])
...
async def cmd_status(m: Message) -> None:
    ...
    for j in rows:
        ...
        who = (f"\n  👤 {j.buzzcast_nick or '—'} (ID: <code>{j.buzzcast_user_id}</code>)"
               if j.buzzcast_user_id else "")
        lines.append(
...
def _collect_server_stats() -> dict:
    """Снимок реальных характеристик сервера (блокирующий — звать через to_thread)."""
...
async def cmd_server(m: Message) -> None:
    """Показать полные реальные характеристики сервера."""
    try:
        s = await asyncio.to_thread(_collect_server_stats)
    except Exception as e:
        log.exception("server stats failed")
        await m.answer(f"❌ Не удалось снять характеристики: {e}")
        return

    active_jobs = len(storage.list_all_active())
    load1, load5, load15 = s["load"]
    load_str = (f"{load1:.2f} / {load5:.2f} / {load15:.2f}"
                if load1 is not None else "n/a")
    freq_str = f" @ {s['cpu_freq']:.0f} MHz" if s["cpu_freq"] else ""
    vm, sm, du, net = s["mem"], s["swap"], s["disk"], s["net"]

    lines = [
        "🖥 <b>Состояние сервера</b>",
        "",
        f"🏷 Хост: <code>{s['hostname']}</code>",
        f"🐧 ОС: {s['os']} ({s['arch']})",
        f"🐍 Python: {s['python']}",
        f"⏱ Uptime: {_human_uptime(s['uptime'])}",
        f"🕓 Запущен: {s['boot_time'].strftime('%Y-%m-%d %H:%M UTC')}",
        "",
        "<b>⚙️ CPU</b>",
        f"• Ядра: {s['cpu_phys'] or '?'} физ. / {s['cpu_logical'] or '?'} лог.{freq_str}",
        f"• Загрузка: {s['cpu_percent']:.0f}%",
        f"• Load avg (1/5/15м): {load_str}",
        "",
        "<b>🧠 Память</b>",
        f"• RAM: {_human_bytes(vm.used)} / {_human_bytes(vm.total)} ({vm.percent:.0f}%)",
        f"• Доступно: {_human_bytes(vm.available)}",
        f"• Swap: {_human_bytes(sm.used)} / {_human_bytes(sm.total)} ({sm.percent:.0f}%)",
        "",
        "<b>💾 Диск (/)</b>",
        f"• {_human_bytes(du.used)} / {_human_bytes(du.total)} ({du.percent:.0f}%)",
        f"• Свободно: {_human_bytes(du.free)}",
        "",
        "<b>🌐 Сеть (с загрузки)</b>",
        f"• ↑ {_human_bytes(net.bytes_sent)}  ↓ {_human_bytes(net.bytes_recv)}",
        "",
        f"<b>📋 Задачи ARGUS:</b> активных {active_jobs}",
    ]
    await m.answer("\n".join(lines))
...
async def cmd_buzzcast(m: Message, command: CommandObject) -> None:
    ...
    try:
        async with aiohttp.ClientSession() as session:
            ...
            _pending[user_id] = Pending(url=stream_url, buzzcast_id=buzzcast_id, nick=nick)
            sent = await m.answer(
...
async def cmd_monitoring(m: Message, command: CommandObject) -> None:
    ...
    if not arg:
        ...
        for entry in entries:
            ...
            lines.append(
                f"• <code>{entry.buzzcast_user_id}</code>"
                f"{(' <b>' + entry.nick + '</b>') if entry.nick else ''} {status}{job_info}\n"
    ...
    try:
        async with aiohttp.ClientSession() as session:
            ...
            _pending[user_id] = Pending(url=f"buzzcast:{buzzcast_id}",
                                        buzzcast_id=buzzcast_id, nick=nick)
            sent = await m.answer(
...
async def on_monitoring_delete(cq: CallbackQuery) -> None:
    ...
    for e in entries:
        ...
        lines.append(
            f"• <code>{e.buzzcast_user_id}</code>"
            f"{(' <b>' + e.nick + '</b>') if e.nick else ''} {status}{job_info}\n"
...
async def on_hoster(cq: CallbackQuery) -> None:
    ...
    pj.hosters = {
        ...
        "tempshare": ["tempshare"],
        "gofile": ["gofile"],
        "both": ["ranoz", "tempshare"],
        "all": ["ranoz", "tempshare", "gofile"],
    }[choice]

    label_map = {"ranoz": "Ranoz", "tempshare": "Tempshare", "gofile": "Gofile"}
...
async def on_quality(cq: CallbackQuery) -> None:
    ...
    if choice == "cancel":
        ...
        await cq.answer()
        return
    is_size = choice.startswith("size:")
    if is_size:
        try:
            mb = int(choice.split(":", 1)[1])
        except ValueError:
            await cq.answer("Некорректный размер.", show_alert=True)
            return
        if mb not in config.SIZE_TARGETS_MB:
            await cq.answer("Некорректный размер.", show_alert=True)
            return
    elif choice not in config.QUALITY_PRESETS:
    ...
    if "tempshare" in pj.hosters:
        ...
        await cq.message.edit_text(
            ...
            f"🗜 Качество: {config.quality_label(choice)}\n\n"
    ...
    if pj.url.startswith("buzzcast:"):
        ...
        await cq.message.edit_text(
            ...
            f"📤 Хостер: {' + '.join(pj.hosters)}\n"
            f"🗜 Качество: {config.quality_label(pj.quality)}\n\n"
...
async def _start_job(bot: Bot, cq: CallbackQuery, pj: Pending) -> None:
    ...
    q_label = config.quality_label(pj.quality)  # type: ignore
    ...
    storage.insert(storage.Job(
        ...
        status="pending",
        buzzcast_user_id=pj.buzzcast_id or "",
        buzzcast_nick=pj.nick or "",
    ))

    summary_lines = [
        f"⏳ Запускаю запись.  ID: <code>{job_id}</code>",
    ]
    subj = _subject_line(pj.buzzcast_id, pj.nick)
    if subj:
        summary_lines.append(subj)
    summary_lines += [
...
async def _run_job(bot: Bot, job_id: str, pj: Pending,
    ...
    if result.target_mb:
        ok_mark = "✅" if result.final_size <= result.target_mb * 1_000_000 else "⚠️"
        lines.append(f"🎯 Цель была ≤{result.target_mb} МБ {ok_mark}")
    # Сколько кусков реально сжалось через ezgif
    if result.seg_total:
        ok_segs = result.seg_total - result.seg_fallback
        lines.append(f"🗜 Сжато через ezgif: {ok_segs}/{result.seg_total} кусков")
        if result.seg_fallback:
            lines.append(
                f"⚠️ <i>{result.seg_fallback} кус(ок/ка/ков) ezgif не осилил "
                f"(оставлены оригиналом). Обычно помогает повтор задачи или "
                f"меньший размер сегмента (EZGIF_SEG_MAX_MB).</i>"
            )
    if result.raw_size and result.final_size >= result.raw_size:
        lines.append(
            "⚠️ <i>Сжатие не дало эффекта — ezgif не справился со всеми "
            "сегментами. Проверь логи: <code>journalctl -u argus-bot -n 200</code></i>"
        )
    lines.append("")
                ...
                for entry in entries:
                    try:
                        ...
                        storage.insert(storage.Job(
                            ...
                            status="pending",
                            buzzcast_user_id=entry.buzzcast_user_id,
                            buzzcast_nick=nick,
                        ))
                        ...
                        pj = Pending(
                            ...
                            ts_duration=entry.ts_duration,
                            chat_id=entry.chat_id,
                            buzzcast_id=entry.buzzcast_user_id,
                            nick=nick,
                        )
                        registry = ffwrap.ProcRegistry()
                        task = asyncio.create_task(_run_job(bot, job_id, pj, registry))
...
async def main() -> None:
    ...
    # Slash-команды в меню Telegram
    await bot.set_my_commands([
        BotCommand(command="start",      description="Начало / помощь"),
        BotCommand(command="buzzcast",   description="Записать стрим BuzzCast по ID"),
        BotCommand(command="monitoring", description="Мониторинг стримеров"),
        BotCommand(command="status",     description="Активные задачи"),
        BotCommand(command="server",     description="Характеристики сервера"),
        BotCommand(command="cancel",     description="Отменить задачу"),
        BotCommand(command="cleanup",    description="Очистить диск"),
    ])
