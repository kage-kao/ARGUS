...
CREATE TABLE IF NOT EXISTS jobs (
    ...
    buzzcast_user_id TEXT DEFAULT '', -- BuzzCast streamer id (if any)
    buzzcast_nick    TEXT DEFAULT '', -- BuzzCast streamer nickname (if any)
...
class Job:
    ...
    def from_row(cls, row: sqlite3.Row) -> "Job":
        ...
        return cls(
            ...
            buzzcast_user_id=(row["buzzcast_user_id"] if "buzzcast_user_id" in keys else "") or "",
            buzzcast_nick=(row["buzzcast_nick"] if "buzzcast_nick" in keys else "") or "",
...
def init() -> None:
    with _conn() as c:
        ...
        # Лёгкие миграции для уже существующих БД (новые колонки).
        for table, col, decl in (
            ("jobs", "buzzcast_user_id", "TEXT DEFAULT ''"),
            ("jobs", "buzzcast_nick", "TEXT DEFAULT ''"),
            ("monitoring", "nick", "TEXT DEFAULT ''"),
        ):
            try:
    ...
    with _conn() as c:
        c.execute(
            """INSERT INTO jobs(id,user_id,chat_id,url,hosters,quality,
                                ...
                                buzzcast_user_id,buzzcast_nick,
            ...
            (job.id, job.user_id, job.chat_id, job.url,
             ...
             job.buzzcast_user_id, job.buzzcast_nick,
...
class MonitoringEntry:
    ...
    def from_row(cls, row: sqlite3.Row) -> "MonitoringEntry":
        ...
        return cls(
            ...
            quality=row["quality"],
            ts_duration=row["ts_duration"],
            nick=(row["nick"] if "nick" in keys else "") or "",
...
def monitoring_add(user_id: int, chat_id: int, buzzcast_user_id: str,
                   hosters: list[str], quality: str, ts_duration: int,
                   nick: str = "") -> None:
    """Add or update monitoring entry."""
    now = int(time.time())
    with _conn() as c:
        c.execute(
            """INSERT INTO monitoring(user_id,chat_id,buzzcast_user_id,nick,hosters,quality,
               ...
               ON CONFLICT(user_id,buzzcast_user_id) DO UPDATE SET
                   ...
                   active=1""",
            (user_id, chat_id, buzzcast_user_id, nick, json.dumps(hosters),
