...
async def _ffmpeg_record(url: str, out_path: Path, log_path: Path,
    ...
    Сама запись остаётся lossless (`-c copy`). Битые таймстампы источника
    (из-за которых длительность раздувается до десятков часов) чинятся
    ОТДЕЛЬНЫМ детерминированным шагом `normalize_recording()` уже после записи —
    он измеряет реальную длительность по числу видео-пакетов и при
    необходимости пересобирает таймстампы.
    """
...
async def _probe_fps(path: Path) -> float:
    """Реальный fps видеопотока (r_frame_rate). 0.0 если не удалось."""
    rc, out, _ = await _run([
        "ffprobe", "-v", "error", "-select_streams", "v:0",
...
async def probe_true_duration(path: Path) -> float:
    """Истинная длительность ВИДЕО = (число видео-пакетов / fps).

    Считает заголовки пакетов (`-count_packets`, без декодирования) — поэтому
    результат НЕ зависит от битой метаданной длительности контейнера и от
    мусорных таймстампов. Это эталон, по которому мы понимаем, что у файла
    «хвост» (контейнер показывает сильно больше, чем есть на самом деле)."""
    rc, out, _ = await _run([
    ...
    if rc != 0:
        return 0.0
    try:
        packets = int(out.decode().strip().split(",")[0])
    except Exception:
        return 0.0
    fps = await _probe_fps(path)
...
async def normalize_recording(raw: Path,
                              registry: "ProcRegistry | None" = None) -> Path:
    """Гарантированно убрать «хвост»/гигантскую длительность записи.

    Эскалация (от самого дешёвого к самому надёжному):
      1. лёгкий ремукс: +genpts + сброс битой метаданной контейнера (lossless);
      2. копия видео + переэнкод аудио с aresample=async=1 (чинит аудио-разрыв,
         видео остаётся lossless);
      3. крайний случай: полный CFR-переэнкод — раскладывает кадры по фиксиро-
         ванному fps, полностью игнорируя битые таймстампы (точно убирает хвост).
    Файл на каждом шаге проверяется по эталонной длительности (числу пакетов).
    Возвращает путь к исправленному файлу (тот же `raw`)."""
    true_dur = await probe_true_duration(raw)
    ...
    async def _try(cmd: List[str], tmp: Path) -> bool:
        rc, _, _ = await _run(cmd, registry=registry)
        if registry is not None and registry.cancelled:
            tmp.unlink(missing_ok=True)
            raise asyncio.CancelledError()
        if rc != 0 or not _has_data(tmp):
            tmp.unlink(missing_ok=True)
            return False
        cont = await probe_duration(tmp)
        td = await probe_true_duration(tmp)
        if _duration_is_sane(cont, td if td > 0 else true_dur):
            os.replace(tmp, raw)
            return True
        tmp.unlink(missing_ok=True)
        return False

    # --- стратегия 1: дешёвый ремукс ---------------------------------------
    fix1 = raw.with_name("fix1.mp4")
    if await _try([
    ...
    fix2 = raw.with_name("fix2.mp4")
    if await _try([
        "ffmpeg", "-nostdin", "-hide_banner", "-loglevel", "error", "-y",
        "-fflags", "+genpts", "-i", str(raw),
        "-map", "0:v:0", "-map", "0:a:0?",
    ...
    # setpts=N/(fps*TB) перекладывает кадры подряд (по их номеру), полностью
    # игнорируя битые таймстампы источника → разрыв ВЫРЕЗАЕТСЯ (а не заполня-
    # ется дублями, как сделал бы -vsync cfr). asetpts делает то же для аудио.
    fix3 = raw.with_name("fix3.mp4")
    if await _try([
        "ffmpeg", "-nostdin", "-hide_banner", "-loglevel", "error", "-y",
        "-fflags", "+genpts", "-i", str(raw),
        "-map", "0:v:0", "-map", "0:a:0?",
        "-vf", f"setpts=N/({fps_arg}*TB)",
