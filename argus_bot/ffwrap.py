...
async def yt_dlp_record(url: str, out_path: Path, log_path: Path,
    ...
    # yt-dlp may return non-zero on graceful stream end; we tolerate this if
    # something was actually recorded.
    if (not out_path.exists() or out_path.stat().st_size == 0) and rc != 0:
