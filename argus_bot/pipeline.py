...
async def run_pipeline(job_id: str, stream_url: str, hosters: List[str],
    ...
    try:
        ...
        # ---------- Normalize duration (fix «хвост»/битые таймстампы) -------
        await _maybe(progress, "🛠 Проверяю и чиню длительность записи...")
        raw = await ffwrap.normalize_recording(raw, registry=registry)
        if registry.cancelled:
            raise asyncio.CancelledError()
        ...
        # ssl=False — отключаем строгую проверку TLS: некоторые хостеры
        # (например ranoz.gg) отдают TLSV1_ALERT_INTERNAL_ERROR на handshake.
