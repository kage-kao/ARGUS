...
| `/cleanup` | вручную подмести `work/` — удалить осиротевшие папки от крашей |

## Авто-очистка диска

Запись стрима может весить десятки ГБ — поэтому `work/<job_id>/` чистится
агрессивно и автоматически:

1. **После успеха / отмены / ошибки** — папка задачи удаляется в `finally`-блоке
   пайплайна (до этого `failed` задачи копились на диске).
2. **При старте бота** — после `mark_stale_interrupted` сканируется `work/`,
   и любая папка, чей `job_id` не активен в БД, удаляется. Это спасает от:
   `kill -9`, OOM, сбоя питания, `systemctl restart` посреди записи.
3. **Фоновый воркер** — каждые 15 минут повторяет шаг 2 (с 60-сек грейс-периодом,
   чтобы не задеть только что созданные папки до `INSERT` в БД).

При `failed` хвост `ytdlp.log` (≤4 КБ) сохраняется в
`/opt/argus-bot/errors/<job_id>.log` для постмортема — основной мусор уходит,
диагностика остаётся.
