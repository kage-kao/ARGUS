...
apt-get update
apt-get install -y python3-flask python3-gunicorn ffmpeg curl -t bookworm-backports yt-dlp
...
ExecStart=/usr/bin/gunicorn --workers 2 --bind 0.0.0.0:5000 --chdir $APP_DIR app:app
Restart=always
RestartSec=3
